...
- **운영 방식 확정(사용자 결정)**: 상위 폴더 훅 등록은 하지 않는다. **RAG 작업 세션은
  `cd ~/project/crypto && claude` 로 crypto 폴더에서 시작**할 것 → 기존 훅이 원래 설계대로 자동
  동작하고, crypto/CLAUDE.md(이 프로젝트 지침)도 올바로 로드된다. 상위 폴더에서 켠 세션이 있었다면
  훅 스크립트 수동 실행으로 백필 가능(가드가 무관 세션을 걸러줌).

## 2026-07-24 — 전체 코드 리뷰(에이전트 2방향) 기반 리팩토링

### 정합성 수정(높은 확신)
- **경로 가드 버그**: CONFIG 경로가 상대('data')일 때 /srcimg·/feedback/image 가드가
  상대 vs 절대 startswith 비교로 **항상 404** → _safe_path(양쪽 절대화+컨테인먼트)로 교체.
  /preview(render_file)엔 가드가 아예 없던 경로 순회 구멍도 봉쇄.
- **인덱서 문서 소실**: 변경 파일의 옛 청크를 파싱 '전에' 삭제 → 일시 파싱 실패 시 문서가
  검색에서 사라짐 → 새 청크 등록 성공 후에만 옛 청크 제거(파일 단위 교체).
- **예약폴더 오배제**: chroma/feedback 를 이름(basename)으로 제외 → 사용자 문서 폴더가
  같은 이름이면 조용히 누락 → 절대경로 기준 판정.
- **LLM 폴백 미작동**: urlopen 읽기 타임아웃(TimeoutError)이 URLError에 안 잡혀 폴백 안 탐
  → except에 TimeoutError/OSError 추가.
- 오류 답변 미저장(새로고침 시 소실), 대화 저장 원자화(tmp→os.replace), 개선기록 이미지
  파일명 충돌(순번→난수), 세션 무한 누적(재로그인 정리+상한 200), 대화 삭제 시 세션 잔재 정리,
  파이프라인 이중 빌드 방지 락, 임베딩 배치 전체 차원 검증.

### 속도 개선
- **로그 대화**: 명령별 필드표(schemas)를 질문마다 전체 청크 재조회+재파싱하던 것을
  **첨부 시 1회 계산해 conv['log']에 캐시**(구버전 첨부는 첫 질문 때 1회 계산 후 저장).
- **인라인 근거 이미지**: PDF 페이지를 매 요청 재렌더 → 서버 LRU 캐시(64) +
  Cache-Control(private, 1일) → 대화 재방문 시 즉시 표시.
- **콜드스타트 제거**: lifespan 워밍업 — 서버 시작 직후 파이프라인(BM25·kiwi·리랭커)을
  백그라운드 로드 → 첫 질문이 로드 대기 없이 응답.
- [속도] 로그에 '(인용 재작성 포함)' 표시 — grounding 재작성이 생성시간 2배의 원인인지 식별 가능.

### 무음 예외 → 경고 로그(품질 저하 가시화)
- kiwipiepy 로드 실패(한국어 BM25 반토막), 명세 청크 조회 실패, 명세 검색 실패,
  PDF 표 추출 실패(페이지 명시), 로그 전처리 정규식 오류, 대화 요약 실패.

### 검증
- 코어 14/14, TestClient 스모크(chat/srcimg캐시/경로차단/admin/feedback/세션정리),
  증분 인덱싱 회귀, grounding 재작성 마커, logchat 캐시 재사용(재조회 0회).

### 보류(후속 후보로 기록만)
- BM25 전체 재구축(증분 업로드마다) → 증분화, list_conversations O(n) 전체 JSON 로드,
  동시 탭 저장 병합(파일락), 취소 토큰 요청별 분리, _complete_tables $in 일괄 조회.
