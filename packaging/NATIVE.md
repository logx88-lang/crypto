...
> ✅ **권장(사내 브라우저에서 파일 업로드/다운로드가 막힐 때의 근본 해법)**: 신규 예쁜 웹 UI(:8502)를
> 그대로 감싸는 **WebView2 데스크톱 클라이언트** = `packaging/client/`(rag-client.exe). 네이티브 앱이라
> 브라우저 정책과 무관하게 파일·클립보드가 동작한다. 빌드·배포는 `packaging/client/README.md` 참고.
> 아래(이 문서)는 **Tkinter 기반 구형 클라이언트(:8600 API)** 로, UI가 단순한 대체안이다.
