...
## 7. HEX 로그 분석 워크플로우 (핵심)

1. **후보 검색**: 업로드 HEX + 질문(장비 단서)로 `doc_type=protocol_spec` 중 관련 명세 후보 하이브리드 검색.
2. **확인 단계 (필수·자동 추측 금지)**: 후보를 UI에 제시, **"이 문서의 프로토콜이 맞습니까?"**
   사용자 확정 전 해석 금지.
3. **해석·분석**: 확정 명세의 **패킷 구조·필드·명령 코드**를 컨텍스트로 HEX 해석·답변.

### HEX 파서 (사용자 제공 포맷 반영)
저장 포맷이 다양(.txt/.dat 등). 관측된 형태:
```
07/08 00:45:05 [00]-[00]      # 타임스탬프 + [] 바이트 + '-' 구분자
07/08 00:45:05 [00][00]       # 타임스탬프 + [] 바이트
[00][00][ab][cd]              # 타임스탬프 없음, [] 바이트
```
- **라인 정규화**: 선택적 선행 타임스탬프(`\d{2}/\d{2} \d{2}:\d{2}:\d{2}`) 제거.
- **바이트 추출**: `\[([0-9a-fA-F]{2})\]` 로 토큰화, `-` 등 구분자 무시 → 바이트 스트림 복원.
- **바이너리 .dat 폴백**: `[]` 패턴이 없고 비출력 바이트 위주면 원시 바이트를 hex로 직접 해석.
- **커맨드 매칭**: 명세(문서당 커맨드 10~30개)의 명령 코드로 프레임 경계·명령 식별.

### 고려사항
- **컨텍스트 초과**: 패킷 경계 분할·질문 관련 구간 추출·요약→상세 2단계. 명세 청크를 컨텍스트에 우선 고정.
- **명세 표 참조 정확도**: 파싱 시 패킷 정의 표를 구조 메타로 보존해 필드 오프셋/폭 정확 인용.
- **UI 모드 분리**: HEX 분석 탭 별도, 파일 업로드 시에만 활성화, 확인 단계 필수 개입.
